...
Wheel::Wheel(volatile uint8_t &port, uint8_t speedPin, uint8_t dirPin, uint8_t speedOffset) : Component(port, speedPin, dirPin, OUT),speedOffset_(speedOffset)
{
    //Initialize PWM
...
void Wheel::set(uint8_t speed, Direction dir)
{
    ...
    if (getSpeedPin() == OCR1A_PIN)
        OCR1A = speed + speedOffset_;
    else if (getSpeedPin() == OCR1B_PIN)
        OCR1B = speed + speedOffset_;
